...
**Reason**: Replaced by pattern groups in the ignore-policy file with explicit `filterType` (`regex`, `prefix`, `suffix`).
**Migration**: Move prefix rules into `patterns` groups in `ignored-repos.yaml` or `ignored-repos.json`; remove any operator-config regex keys if previously documented.
